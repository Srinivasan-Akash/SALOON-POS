...
import { customStyles, customStylesV3, dataURLtoBlob } from "../../utils/utils";
...
export function PayNewBill({ data }: { data: any }) {
  const { services, employees } = useDataContext();
  ...
  const staff: Option[] = employees.map((service: any) => ({
  ...
  const [paymentStatusInput, setPaymentStatusInput] = useState<Option | null>(
    null
  );
    ...
    if (printWindow) {
      try {
        ...
        printWindow.document.write(
          "<html><head><title>Invoice Preview</title></head><body>"
        );
        ...
        const response: { $id: string } = await new Promise(
          (resolve, reject) => {
            storage
              .createFile(invoicesBucket, id, file)
              .then(resolve)
              .catch(reject);
          }
        );
...
Thank you for choosing our services! If you have any questions, feel free to reach out. 😊`;
        ...
        const messageResponse = await fetch(
          `${whatsapp_endpoint}/send-message?number=${
            data.phone
  ...
  const handleQuantityChange = (
    ...
    if (updatedFormRows[index].selectedService) {
      updatedFormRows[index].price =
        updatedFormRows[index].selectedService.price * quantity;
    }
      ...
      <div className="form">
        <div className="headline">
          ...
          <div style={{ display: "flex", gap: ".5em" }}>
            <div onClick={removeFormRow}>
              <FaMinus size={12} />
            </div>
            <div onClick={addNewFormRow}>
              <FaPlus size={12} />
            </div>
          </div>
        </div>
            ...
            <div className="formRow" key={index}>
              ...
              <div className="dropdown">
                <Select
                  ...
                  onChange={(selectedOption) =>
                    handleStaffChange(selectedOption, index)
                  }
        ...
        <div className="offers">
          <div className="discount">
            <input
              type="text"
              ref={discountField}
              placeholder="Enter Desired Discount in % or Rs"
            />
            <button onClick={() => setDiscount(discountField.current.value)}>
              ADD DISCOUNT
            </button>
          </div>
        </div>        
        ...
        <div className="detailsToKnow">
          ...
          <div>
            <Select
              ...
              onChange={(selectedOption) =>
                setPaymentStatusInput(selectedOption)
              }
        ...
        <button className="total" disabled={isLoading} onClick={addTransaction}>
          {isLoading ? (
            <>
